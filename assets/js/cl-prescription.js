        ...
        success: function(data, status){
            ...
            if(status != '0'){
				...
				if(data.status == "error") {
					if(jQuery(".sp-prescription-box-elements").find('span.invalid').length == 0) {
						jQuery(".sp-prescription-box-elements").prepend('<span class="error top invalid">* The parameters you selected for your ' + eyeType + ' eye are not available.</span>'); 
					}
					else {		
						jQuery(".sp-prescription-box-elements").find('span.invalid').html('* The parameters you selected for your ' + eyeType + ' eye are not available.').fadeIn("fast");
					} 
				}
				else {
					jQuery(".sp-prescription-box-elements").find('span.invalid').fadeOut("fast");
				}
				jQuery('input[name="upccode"]').val(data.upccode);	
...
jQuery( document ).ready(function($) {
	...
	$(".lg-custom-drop").each(function(){
		...
		$(this).find("input").val(); */ 
		if($(this).hasClass("single_val_attr")) {
		}
		else {	
	...
	$(".close-btn").on("click", function(e){
		...
		setTimeout(function(){
			...
			$("#eye-selection").show();
			$("#continue-btn").show();
			$(".fl-page .fl-builder-content").fadeIn(200);
		}, 200);	
	...
	$("#continue-btn a").on("click", function(e){
		...
		let refresh = window.location.protocol + "//" + window.location.host + window.location.pathname + '?prtype=cl';    
		...
		if(!jQuery('input[name="attribute_pa_eye-right"]').is(":checked") && !jQuery('input[name="attribute_pa_eye-left"]').is(":checked") ) {
			if($("#eye-selection").find('span.eyeerror').length == 0) {
				$("#eye-selection").append('<span class="eyeerror top">Please select an eye.</span>');
			}
			else {		
				$("#eye-selection").find('span.eyeerror').html("Please select an eye."); 
				$("#eye-selection").find('span.eyeerror').fadeIn("fast"); 
			}	
			$("#eye-selection").find('span.error').hide();
			isValid = false;			
		}
		else if(right_eye_qty == 0 && left_eye_qty == 0) {
			...
			$("#eye-selection").find('span.eyeerror').hide();
			isValid = false;			
		}		
		else if(!jQuery('input[name="attribute_pa_eye-right"]').is(":checked") && jQuery('input[name="attribute_pa_eye-left"]').is(":checked") && left_eye_qty == 0 ) {
			if($("#eye-selection").find('span.eyeerror').length == 0) {
				$("#eye-selection").append('<span class="eyeerror top">Please select a valid quantity for selected eye.</span>');
			}
			else {		
				$("#eye-selection").find('span.eyeerror').html("Please select a valid quantity for selected eye."); 
				$("#eye-selection").find('span.eyeerror').fadeIn("fast"); 
			}	
			isValid = false;
		}
		else if(jQuery('input[name="attribute_pa_eye-right"]').is(":checked") && !jQuery('input[name="attribute_pa_eye-left"]').is(":checked") && right_eye_qty == 0 ) {
			if($("#eye-selection").find('span.eyeerror').length == 0) {
				$("#eye-selection").append('<span class="eyeerror top">Please select a valid quantity for selected eye.</span>');
			}
			else {		
		...
		if(isValid) {	
			$(".fl-page div.fl-builder-content").hide(); 
			...
			jQuery('html, body').animate({ scrollTop: jQuery("#cl-steps-wrapper").offset().top - 25}, 500);
			prev_step = "";
	...
	/*===== File Upload =====*/
	$('.upload_img input[type="file"]').on("change", function() {	 	 
		var _this_ = this;
